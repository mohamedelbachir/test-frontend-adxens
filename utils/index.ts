...
export function computeMetrics(data: any) {
  ...
  const sum = (arr: Tx[]) => arr.reduce((s, t) => s + t.amount, 0);
  ...
  const currentStart = new Date(data.periods.current.start);
  const currentEnd = new Date(data.periods.current.end);
  ...
  txs.forEach((t) => {
    ...
    if (!inRange(t.date, currentStart, currentEnd)) return;
    ...
    if (t.category === "ad_spending") {
      stackedMap[label].adSpending += t.amount;
    }

    if (t.category === "allocation") {
      stackedMap[label].allocation += t.amount;
    }
  });

  const stackedChart = Object.values(stackedMap)
    .sort((a, b) => a.ts - b.ts)
    .map(({ date, adSpending, allocation }) => ({
      date,
      adSpending,
      allocation,
    }));
